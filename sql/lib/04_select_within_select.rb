...
def sparse_continents
  ...
  execute(<<-SQL)
  select name, continent, population from countries x where 25000000 > all(select population from countries y where x.continent = y.continent and population > 0)
  SQL
end

def test
  execute(<<-SQL)
  select name from countries where population < 25000000
  SQL
end
